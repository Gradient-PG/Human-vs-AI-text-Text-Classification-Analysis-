...
\todo{If a second base/non-RLHF generator is added before submission, update count to 6 here, add it to the generator list, and update the layer-asymmetry claim in §\ref{sec:crossgen-layers} from a single-data-point to a pattern.}

RAID~\citep{dugan2024raid} provides human-written and machine-generated texts across 11 generators and 8 domains. We use six of the eight domains: abstracts, books, news, Reddit posts, reviews, and Wikipedia. We select five generators for architectural diversity: GPT-4 (\texttt{gpt4}), GPT-2 (\texttt{gpt2}), Mistral-7B-Instruct (\texttt{mistral-chat}), Llama-2-Chat (\texttt{llama-chat}), and Cohere Command (\texttt{cohere-chat}), spanning one pure-base generator (gpt2), one SFT-only generator (mistral-chat), and three SFT+RLHF/preference-trained generators (gpt4, llama-chat, cohere-chat), across four model families. We subsample 7{,}500 examples per generator (3{,}750 human, 3{,}750 AI) balanced across the six RAID domains, yielding 37{,}500 samples in total.
