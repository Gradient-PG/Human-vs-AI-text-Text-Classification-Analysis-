...
Per-generator geometric diagnostics of the stable sets (CAV cosine alignment, mean-difference norm, and probe weight norm) are provided in Appendix~\ref{app:cav} (Table~\ref{tab:cav}) and corroborate the cross-generator differences explored in §\ref{sec:results-crossgen}: instruction-tuned generators show higher cosine alignment and more compact weight vectors, while Cohere exhibits a diffuse signal geometry that predicts its outlier behaviour throughout.
...
\toprule
 & GPT-4 & GPT-2 & MPT & Mistral & LLaMA & Cohere \\
\midrule
Flip rate (\%) & $1.23_{\pm 0.34}$ & $3.13_{\pm 0.56}$ & $1.52_{\pm 0.22}$ & $1.73_{\pm 0.28}$ & $1.07_{\pm 0.38}$ & $8.15_{\pm 1.32}$ \\
\bottomrule
\end{tabular}
\caption{Selected flip rate (\%) at $k = \text{full}$ (15-cell means $\pm$ std). Full $k$-sweep in Appendix~\ref{app:patching-sweep}.}
...
The absence of contradiction follows from redundant distribution: the L2 probe retains 9,000+ correlated features, making ablation of $\leq$1\% a weak test; mean ablation also moves activations off-manifold~\citep{heimersheim2024patching}, so we treat this as a conservative lower bound.
...
Pairwise Jaccard similarity between stable sets (Table~\ref{tab:jaccard} in Appendix~\ref{app:jaccard}) ranges from 0.00 to 0.24 in absolute terms, consistent with generator-specific circuits. However, the appropriate baseline is not zero but the expected Jaccard for two random sets of the same sizes drawn from 9,216 features, which is approximately 0.003 per pair. Relative to this null, the mean observed Jaccard of 0.073 is $\sim$24$\times$ above chance, indicating a partial shared substrate despite the generator-specific structure.
...
Neurons appearing in the stable sets of at least 3 of the 6 generators (minimal majority threshold) form a \textit{core} set of 17 neurons, of which 10 (59\%) reside in layer~12, far exceeding any individual generator's L12 share (3.8--36\%), with the remaining 7 scattered across layers 1, 2, 5, 7, 8, and 11. This concentration suggests that BERT's layer-12 representations are the primary cross-generator site for AI-detection signal, with each generator's additional stable neurons forming a generator-specific peripheral set on top.
...
\paragraph{Protocol.}
We partition all 11 RAID generators into five families: \texttt{gpt}, \texttt{cohere}, \texttt{llama}, \texttt{mistral}, and \texttt{mpt}. For each held-out family we run the full sparse-probe pipeline on the remaining generators and evaluate two probes on the held-out set: (i)~\textbf{Full L2} on all 9{,}216 features, and (ii)~\textbf{Sel\,L2} on only the stable neurons identified from training families.
...
Cohere-Command-Chat's consistently diffuse geometry (lowest CAV cosine, largest probe weights, highest flip rate, only generator with substantial ablation cost) is internally coherent: the signal is encoded with small per-feature magnitude, the probe compensates via large weights, and the decision boundary is near enough that both patching and ablation affect it. The two pure-base generators (GPT-2, MPT-7B) form a coherent cluster with near-zero overlap with instruction-tuned generators yet share 12 neurons with each other (J\,=\,0.120, 40$\times$ chance), both avoid layer~12 ($\leq$13.3\% vs.\ 30--36\% for instruction-tuned), and both show the two largest restricted-probe gaps (Appendix~\ref{app:restricted}). This consistency across two independent base models supports the interpretation that post-training alignment leaves a layer-12 footprint absent in base-only decoding.
...
Frozen \bertbase already encodes sparse, causally sufficient AI-detection circuits. An L1-regularised probe stably selects 45--62 neurons ($<$1\% of 9,216 CLS features) whose activation patching flips human predictions to AI at 1.07--8.15\%, 10--16$\times$ above a random baseline, while mean-ablation of the same set causes $\leq$1.1\,pp accuracy loss on five of six generators. The joint pattern, large flip rates alongside negligible ablation drops, is the signature of a signal that is \textit{easy to inject, hard to erase}: redundantly distributed across the full representation but concentrated enough to be causally sufficient.

Cross-generator analysis reveals a bipartite structure: instruction-tuned generators concentrate 30--36\% of stable neurons in BERT's final layer, while both pure-base generators fall below 14\%, consistent with a layer-12 footprint of post-training alignment. Leave-one-family-out evaluation confirms that neurons identified from training families retain 86--94\% of the full-feature ceiling on entirely held-out generator families, establishing that the discovered circuits are not artefacts of the specific generators studied.
...
\caption{Overview of the experimental pipeline. Frozen \bertbase CLS activations are concatenated across all 12 layers, yielding $\mathbf{z}\in\mathbb{R}^{9{,}216}$. An L1-regularised probe ($C\!=\!0.005$) selects a sparse candidate set per (fold, seed) cell; neurons stable in ${\geq}50\%$ of all 15 cells become the \textit{stable set}~$\mathcal{S}$ (45--62 neurons per generator). An independent L2 probe trained on all 9{,}216 features is frozen and reused as the decision function in both experiments. \textbf{Left branch} (\S\ref{sec:results-necessity}): mean ablation tests necessity (accuracy drops $\leq\!1.2$\,pp on 5/6 generators). \textbf{Right branch} (\S\ref{sec:results-patching}): activation patching tests causal sufficiency: 1--8\% of human samples flip to AI, $10\text{--}16\times$ above a random baseline.}
\label{fig:pipeline}
\end{figure*}
...
The pattern splits cleanly along the base/instruction-tuned axis. Instruction-tuned generators (GPT-4, Mistral, LLaMA) show gaps of $\leq$1.2\,pp, indicating that their stable neurons are nearly sufficient on their own and that the full-feature probe does not heavily rely on non-stable dimensions. The two pure-base generators and Cohere-Command-Chat show gaps of 3.6--4.7\,pp, indicating that their full-feature probes lean more on non-stable features, and that ablating the complement is more costly. This is consistent with a more distributed encoding structure for these three generators, complementing the layer-12 avoidance (base generators) and the large ablation sensitivity (Cohere) documented in the main body.
